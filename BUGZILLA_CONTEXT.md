...
| 2026-08-29 | Full Fresh Repository Audit & Phase 5A Completion: Fixed cross-project comment deletion IDOR vulnerability in `comments.py` by enforcing hierarchical `project -> issue -> comment` scoping; fixed `test_jwt_config.py` production config test environment variable isolation; modernized all SQLAlchemy queries to `db.session.get()`; removed unused `bcrypt` from requirements; updated `docs/DEPLOYMENT.md`; implemented Phase 5A issue lifecycle features (inline comment editing with optimistic state, full Kanban board transitions with quick start/review/resolve/reopen controls, and automated resolution state handling); added regression test suites for cross-project comment deletion, comment deletion permissions, admin invariance, complete issue lifecycle E2E, and non-member authorization rejections (13 comprehensive test suites in `test_security_audit_fixes.py`); verified all 65 backend pytest tests pass cleanly against live PostgreSQL (including 15-thread concurrency test); verified frontend typecheck (`tsc --noEmit`), Vite production build, and `npm audit` (0 vulnerabilities). |
| 2026-08-29 | Production & Vercel Readiness Hardening: Upgraded and pinned `Flask-CORS==6.0.5` in `requirements.txt`; implemented fail-closed production `VITE_API_URL` validation in `frontend/src/api/client.ts`; reordered `remove_member` in `projects.py` to acquire parent project row lock (`with_for_update()`) before evaluating caller authorization and member counts; strictly isolated `ProductionConfig.CORS_ORIGINS` to fail closed; added member removal regression tests (expanding suite to 66 tests, 100% passing against live PostgreSQL); verified `npm ci`, `npm run typecheck`, `npm run build`, and `npm audit` (0 vulnerabilities). |
