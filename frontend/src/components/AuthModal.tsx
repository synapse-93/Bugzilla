...
export function AuthModal({ initialMode = 'LOGIN', onClose }: AuthModalProps) {
  ...
  // Clear form on mount and unmount
  useEffect(() => {
    resetForm()
    return () => {
      resetForm()
          ...
          <Card className="border-border bg-card/60 rounded-md shadow-none">
            <CardContent className="p-5 sm:p-6 space-y-4">
              ...
              {authMode === 'NEW_OAUTH_USER' && pendingOAuth ? (
                ...
                <>
                  {/* Mode Switcher Tabs */}
                  <Tabs value={authMode} onValueChange={(val) => handleSwitchMode(val as AuthMode)} className="w-full">
                    <TabsList className="grid w-full grid-cols-2 h-8 rounded-sm bg-muted/40 p-0.5">
                      <TabsTrigger value="LOGIN" className="text-[11.5px] rounded-xs font-medium">
                        Sign In
                      </TabsTrigger>
                      <TabsTrigger value="REGISTER" className="text-[11.5px] rounded-xs font-medium">
                        Create Account
                      </TabsTrigger>
                    </TabsList>
                  </Tabs>
                  ...
                  <Button
                    ...
                    disabled={oauthInitiating || loading}
                    className="w-full justify-center gap-2.5 h-9 text-[12px] border-border hover:bg-muted/40 font-medium rounded-sm"
                  >
                    {oauthInitiating ? (
